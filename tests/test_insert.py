...
def test_series_1D_numeric(s1):
    ...
    # ignore_index
    assert vin(insert(s, 0, s1, ignore_index=True)) == (
        [11, 12, 20, 30],
        [0, 1, 2, 3],
        "A",
    )
    assert vin(insert(s, 1, s1, ignore_index=True)) == (
        [20, 11, 12, 30],
        [0, 1, 2, 3],
        "A",
    )
    assert vin(insert(s, 2, s1, ignore_index=True)) == (
        [20, 30, 11, 12],
        [0, 1, 2, 3],
        "A",
    )
...
def test_series_1D_non_numeric(s1, label):
    ...
    assert vin(insert(s, 0, s1, label=label)) == (
        [11, 12, 20, 30],
        ["x", "y", "b", "c"],
        "A",
    )
    assert vin(insert(s, 1, s1, label=label)) == (
        [20, 11, 12, 30],
        ["b", "x", "y", "c"],
        "A",
    )
    assert vin(insert(s, 2, s1, label=label)) == (
        [20, 30, 11, 12],
        ["b", "c", "x", "y"],
        "A",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            pd.DataFrame(range2d(2, 3) * 10, columns=list("ABC"), index=[5, 6]),
            lib.no_default,
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (range2d(2, 3) * 10, ["x", "y"]),
        (
            pd.DataFrame(range2d(2, 3) * 10, columns=list("ABC"), index=["p", "q"]),
            ["x", "y"],
        ),
        (
            pd.DataFrame(range2d(2, 3) * 10, columns=list("ABC"), index=["x", "y"]),
...
def test_columns():
    ...
    assert vic(df1) == ([[1, 2, 1, 3], [4, 5, 2, 6]], [0, 1], ["A", "B", "D", "C"])
...
def test_append():
    ...
    df1 = append(
        df,
        pd.DataFrame([[7, 8], [9, 10]], columns=["D", "E"], index=["a", "b"]),
        axis=1,
    )
    assert vicn(df1) == (
...
def test_categoricals(col):
    ...
    assert vic(df1) == ([[3, 2, 1, 1], [6, 5, 2, 4]], [0, 1], ["A", "B", "D", "C"])
    df2 = df1.sort_index(axis=1)
    assert vic(df2) == ([[1, 2, 3, 1], [4, 5, 6, 2]], [0, 1], ["C", "B", "A", "D"])
...
def test_unordered():
    ...
    df.columns = pd.CategoricalIndex(
        ["A", "B", "C"], categories=["A", "B", "C"], ordered=False, dtype="category"
    )
