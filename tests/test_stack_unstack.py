...
def test_1Lx1L():
    df = pd.DataFrame([[1, 2, 3], [4, 5, 6]], columns=list("CBA"), index=list("ba"))
    df
...
def test_2Lx2L():
    df = pd.DataFrame(
        ...
        index=from_dict({"K": ("b", "a"), "L": ("d", "c")}),
        columns=from_dict({"M": ("B", "A"), "N": ("D", "C")}),
    )
...
def test_topo():
    df = pd.DataFrame(
        ...
        index=["b", "a"],
        columns=from_dict({"K": list("BBAA"), "L": ["Wed", "Fri", "Mon", "Wed"]}),
    )
