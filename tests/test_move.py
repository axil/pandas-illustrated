...
def test_columns():
    df = pd.DataFrame([[1, 2, 3], [4, 5, 6]], columns=list('ABC'), index=list('ab')); df
    
    assert vic(move(df, 0, 'C')) == ([[3, 1, 2], 
                                      [6, 4, 5]], ['a', 'b'], ['C', 'A', 'B'])

    assert vic(move(df, 1, 'C')) == ([[1, 3, 2], 
                                      [4, 6, 5]], ['a', 'b'], ['A', 'C', 'B'])

    assert vic(move(df, 3, 'A')) == ([[2, 3, 1], 
                                      [5, 6, 4]], ['a', 'b'], ['B', 'C', 'A'])


def test_rows():
    df = pd.DataFrame([[1, 2], [3, 4], [5, 6]], columns=list('AB'), index=list('abc')); df
...
def test_reset_index():
    df = pd.DataFrame([[1, 2], [3, 4], [5, 6]], columns=list('AB')); df
